...
export default {
  async transactionTime(id) {
    const status = await (
      await fetch(`https://${constants.arweaveUrl}/tx/${id}/status`)
    ).json();
    const blockInfo = await (
      await fetch(
        `https://${constants.arweaveUrl}/block/hash/${status.block_indep_hash}`
      )
    ).json();
  ...
  dataPoints(provider, interval, pointsPerInterval) {
    ...
    return interval
      ? providerSnapshot +
          Math.floor(
            pointsPerInterval *
              ((new Date().getTime() - snapshotTimestamp) / interval)
          )
      : 0;
  },
